...
const CompanyCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            ...
            addcompanyname: _entity.addcompanyname,
            addcompanycontactnumber: _entity.addcompanycontactnumber,
            addcompanycontactemail: _entity.addcompanycontactemail,
            addcompanydesc: _entity.addcompanydesc
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div>
                ...
                <div>
                    <p className="m-0" >Company Name:</p>
                    <InputText className="w-full mb-3" value={_entity?.addcompanyname} onChange={(e) => setValByKey("addcompanyname", e.target.value)}  />
                </div>
                <div>
                    <p className="m-0" >Company Contact Number:</p>
                    <InputText className="w-full mb-3" value={_entity?.addcompanycontactnumber} onChange={(e) => setValByKey("addcompanycontactnumber", e.target.value)}  />
                </div>
                <div>
                    <p className="m-0" >Company Contact Email:</p>
                    <InputText className="w-full mb-3" value={_entity?.addcompanycontactemail} onChange={(e) => setValByKey("addcompanycontactemail", e.target.value)}  />
                </div>
                <div>
                    <p className="m-0" >Company Description:</p>
                    <InputText className="w-full mb-3" value={_entity?.addcompanydesc} onChange={(e) => setValByKey("addcompanydesc", e.target.value)}  />
                </div>
